<div class="flex flex-wrap mx-2 mb-3 p-1">
    ...
    <x-forms.input type="tel" name="phone"
        ...
        <x-slot:label_text> Téléphone
            @isset($client_data->phone)
                ( {{ $client_data->phone }} )
            @endisset
        </x-slot:label_text>
    </x-forms.input>
    ...
    <x-forms.input type="text" name="cni" sms-set-value="{{ isset($client_data->cni) ? $client_data->cni : '' }}"
        maxlength="20">
        <x-slot:label_text> Numéro de la carte nationale
            @isset($client_data->cni)
                ( {{ $client_data->cni }} )
            @endisset
        </x-slot:label_text>
    </x-forms.input>
    ...
    <x-forms.input type="text" name="address"
        ...
        >
        <x-slot:label_text> Adresse
            @isset($client_data->address)
                ( {{ $client_data->address }} )
            @endisset
        </x-slot:label_text>
    </x-forms.input>
    ...
    <x-forms.input type="phone" name="profession"
        ...
        <x-slot:label_text> Profession
            @isset($client_data->profession)
